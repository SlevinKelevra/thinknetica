...
class Core
  ...
  def trains_on_station(station)
    get_station(station).trains.each { |train| puts "Number train: #{train.numer}, type #{train.type}, number of wagons #{train.wagon.length}" }
  end

  def see_train(number_train)
    train = @trains_collection.find { |train| train.numer == number_train }
    if train.is_a? PassengerTrain
      train.each_wagon { |wagon, _wagon_number| puts "Wagon: #{wagon.class}, number #{index}, free place #{wagon.free_place}, place #{wagon.busy_place}" }
    elsif train.is_a? CargoTrain
      train.each_wagon { |wagon, _wagon_number| puts "Wagon: #{wagon.class}, number #{index}, free volume #{wagon.free_volume}, volume #{wagon.volume_occupeid}" }
    end
  end
